...
| 7 | Split `MainWindow.axaml` (1,722 lines) into UserControls | enables 2 to 6 | medium |

7 is not cosmetic, there is currently no seam to iterate on. It is worth doing early
enough that 2 to 6 land in files that make sense, but it should follow the token work so the
...
- **Borders had to split into two tokens.** A decorative card border can be low-contrast, but an
  ...
  `border` token managed 1.31 to 1.40. Added `border-input`: `#697080` / `#818A99`.
